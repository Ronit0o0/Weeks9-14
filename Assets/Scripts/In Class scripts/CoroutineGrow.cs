...
public class CoroutineGrow : MonoBehaviour
{
    ...
    {
        float pickASize = Random.Range(1, 5);
        float t = 0;
        while (t < appleGrowDuration)
        {
            pickASize += Time.deltaTime;
            t += Time.deltaTime;
            appleTransform.localScale = Vector3.one * pickASize / appleGrowDuration;
    ...
    {
        //ANY TIME YOU STOP A COROUTINE YOU NEED TO CHECK IF ITS NULL OTHERWISE YOU WILL GET AN ERROR
